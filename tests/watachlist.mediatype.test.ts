...
// @ts-ignore
import {responseWatchlistMovies, responseWatchlistTv} from "./utilities/WatchlistMediaType";
...
describe('Fetch Watchlist Movies associated to an specific accountId', () => {
    it('should retrieve a list of movies', async () => {
        ...
        const response = await api.get('/api/account/10332765/watchlist/movies?language=en-US&page=1&sort_by=created_at.asc')
            .expect(200)
...
describe('Fetch Watchlist TV associated to an specific accountId', () => {
    it('should retrieve a list ot tv', async () => {
        const expectedObject = responseWatchlistTv()

        const response = await api.get('/api/account/10332765/watchlist/tv?language=en-US&page=1&sort_by=created_at.asc')
            .expect(200)
    ...
    it('should manage an incorrect accountId', async () => {
        ...
        const response = await api.get('/api/account/103de7r5/watchlist/tv').expect(400)
    ...
    it('should manage an incorrect page number', async () => {
        ...
        const response = await api.get('/api/account/10332765/watchlist/tv?page=x').expect(400)
